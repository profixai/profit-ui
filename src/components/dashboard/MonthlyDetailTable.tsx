              ...
              <TableRow>
                {([
                  ...
                  <TableHead
                    key={key}
                    onClick={() => toggle(key)}
                    className={`cursor-pointer select-none ${key !== "month" ? "text-right" : ""}`}
                  >
            ...
            <TableBody>
              {withDelta.map((row) => (
                <TableRow key={row.month}>
                  ...
                  <TableCell className="text-right font-mono-data">{fmt(row.revenue)}</TableCell>
                  <TableCell className="text-right font-mono-data">{fmt(row.costs)}</TableCell>
                  <TableCell className="text-right font-mono-data">{fmt(row.gop)}</TableCell>
                  <TableCell className="text-right font-mono-data">{row.gop_margin_pct.toFixed(1)}%</TableCell>
                  <TableCell className={`text-right font-mono-data ${row.marginDelta >= 0 ? "text-positive" : "text-destructive"}`}>
