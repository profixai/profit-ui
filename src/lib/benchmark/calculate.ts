...
export function generateWatchouts(b: BenchmarkResult): Watchout[] {
  ...
  // Region-specific
  b.region.watchouts.forEach((w) =>
    watchouts.push({ severity: "regional", text: w })
  );
  ...
  if (b.sizeClass.code === "micro") {
    watchouts.push({
      severity: "size",
      text: "Microempresa scale shows notably lower EBITDA (17.3%) vs Pequena (32.5%). Consider whether you can grow to 10+ employees or focus on owner-operated efficiency.",
    });
  }
  ...
  if (b.occupancy < 50) {
    watchouts.push({
      ...
      text: `Occupancy of ${b.occupancy.toFixed(1)}% is below 50% — fixed-cost leverage is critical. Focus on revenue management and seasonal pricing.`,
    });
  }
  ...
  if (b.adr < nationalAdr * 0.7) {
    watchouts.push({
      ...
      text: `Your ADR of €${b.adr.toFixed(0)} is well below national average (€${nationalAdr}). Star upgrade or repositioning may unlock 10–20% ADR lift.`,
    });
  }
  ...
  if (b.inputs.rooms < 20) {
    watchouts.push({
      ...
      text: "Hotels with <20 rooms typically have 54%+ OTA share. Aggressive direct-booking strategy is essential — EU DMA now allows lower direct rates than Booking.com.",
    });
  }
  ...
  if (b.region.seasonality_ratio >= 5) {
    watchouts.push({
      severity: "seasonal",
      text: `Extreme seasonality (${b.region.seasonality_ratio}x peak-to-trough). Year-round staff retention, off-season closure, and shoulder-month strategies are critical decisions.`,
    });
  }
  ...
  if (b.pnl.ebitdaMargin < NATIONAL.ebitda_median) {
    watchouts.push({
      severity: "financial",
      text: `Modeled EBITDA margin (${(b.pnl.ebitdaMargin * 100).toFixed(1)}%) is below national median (20.9%). Top-quartile hotels achieve 38%+ — significant improvement room.`,
    });
  }
  ...
  if (b.region.revpar_yoy < 0) {
    watchouts.push({
      severity: "trend",
      text: "Your region is the ONLY one in Portugal with declining RevPAR in 2025. Don't rely on rate growth — focus on cost discipline and ancillary revenue.",
    });
  }
...
export function generateOpportunities(b: BenchmarkResult): Opportunity[] {
  ...
  opps.push({
    ...
    text: `Strongest 2025 RevPAR growth in shoulder months: April +10.9%, January +10.4%. Your region's ALOS is ${b.region.alos} nights — bundle 2-3 night packages for shoulder demand.`,
  });
  ...
  if (b.region.domestic_share > 40) {
    opps.push({
      icon: "🇵🇹",
      title: "Domestic demand engine",
      text: `Your region is ${b.region.domestic_share}% domestic-dependent. Portuguese travel grew +5.4% in 2025 — direct outreach to PT market is high-ROI.`,
    });
  }
  ...
  opps.push({
    icon: "☀️",
    title: "Solar PV payback",
    text: `Your region's solar yield is ${b.region.solar_yield} kWh/kWp/yr — payback in 4–7 years at €800–1,200/kWp. Hotels have ideal daytime load profile (70–90% self-consumption).`,
  });
  ...
  if (parseInt(b.inputs.starCode) >= 3) {
    opps.push({
      icon: "🍽️",
      title: "F&B margin advantage",
      text: "Portugal's 10% IVA on restaurant services (vs 23% standard) gives F&B operations a strong margin advantage. Banquet/catering growth is +9.5% YoY industry-wide.",
    });
  }
  ...
  opps.push({
    ...
    text: "EU DMA (Nov 2024) removed rate parity. You can legally undercut Booking.com on your own site by 5–10%. Direct bookings have ~18% cancellation rate vs OTA ~50%.",
  });
...
export function fmtEur(n: number | null | undefined, decimals = 0): string {
